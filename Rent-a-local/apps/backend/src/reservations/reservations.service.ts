...
export class ReservationsService {
  ...
  for (const c of conflict) {
    if (c.startDate < new Date(endDate) && c.endDate > new Date(startDate)) {
      throw new BadRequestException(
        'Ce local est déjà réservé pour ces dates. La réservation finira le ' +
  ...
  private async checkOverlap(
    ...
    if (overlapping) {
      throw new BadRequestException(
        'Ce local est déjà réservé pour ces dates. La réservation finira le ' + overlapping.endDate.toLocaleDateString('fr-FR'),
      );
    }
  }
