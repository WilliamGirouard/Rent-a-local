...
const locals_factory_1 = require("./locals.factory");
let LocalsService = class LocalsService {
    ...
    async create(dto) {
        const local = locals_factory_1.LocalFactory.create(dto);
    ...
    async findOne(id) {
        ...
        if (!local) {
            throw new common_1.BadRequestException('Local non trouvé.');
        }
        return local;
    }
    async remove(id) {
